...
import { removeArticle, saveArticle } from "../localArticles";
  ...
  return (
    <article className="flex flex-col text-left rounded-md border-1 border-gray-300 dark:border-gray-800 shadow-md justify-between w-1/5 not-lg:w-1/3 gap-2">
      <img
        ...
        alt={article.title}
        className={`size-fit rounded-tl-md rounded-tr-md border-0 dark:text-white ${!article.urlToImage ? "p-2" : ""
          }`}
      />
      <div className="px-2">
      ...
      <div className="flex flex-row pb-8 px-4 gap-2">
        ...
        <Btn onClick={() => {
          ...
          {saved ? <PiBookBookmarkFill/> : <PiBookBookmark />}
        </Btn>
      </div>
    </article>
  )
